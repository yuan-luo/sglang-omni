...
"""Bisect inside attention at layer 0: find where HF and Torch diverge."""
...
@torch.no_grad()
def run_hf(model_path, hf_inputs, enc_out, device, dtype, layer_idx):
    ...
    # Hook on input_layernorm
    def ln_out(module, args, output):
        cap["after_layernorm"] = output.detach().cpu()
    handles.append(layer.input_layernorm.register_forward_hook(ln_out))

    # Hook on q_proj, k_proj
    def qproj_out(module, args, output):
        cap["q_proj_out"] = output.detach().cpu()
    handles.append(layer.self_attn.q_proj.register_forward_hook(qproj_out))
    def kproj_out(module, args, output):
        cap["k_proj_out"] = output.detach().cpu()
    handles.append(layer.self_attn.k_proj.register_forward_hook(kproj_out))
    def vproj_out(module, args, output):
        cap["v_proj_out"] = output.detach().cpu()
    handles.append(layer.self_attn.v_proj.register_forward_hook(vproj_out))

    # Hook on q_norm, k_norm
    def qnorm_out(module, args, output):
        cap["q_norm_out"] = output.detach().cpu()
    handles.append(layer.self_attn.q_norm.register_forward_hook(qnorm_out))
    def knorm_out(module, args, output):
        cap["k_norm_out"] = output.detach().cpu()
    handles.append(layer.self_attn.k_norm.register_forward_hook(knorm_out))

    # Hook on o_proj
    def oproj_out(module, args, output):
        cap["o_proj_out"] = output.detach().cpu()
    handles.append(layer.self_attn.o_proj.register_forward_hook(oproj_out))
    ...
    print(f"[HF] Captured: {list(cap.keys())}")
    for h in handles: h.remove()
    del model; gc.collect(); torch.cuda.empty_cache()
    import time; time.sleep(1); gc.collect(); torch.cuda.empty_cache()
    return cap

@torch.no_grad()
def run_torch(model_path, hf_inputs, enc_out, device, dtype, layer_idx):
    ...
    print("[Torch] Forward ...")
    model(input_ids=ids, attention_mask=mask, **kw)
    print(f"[Torch] Captured: {list(cap.keys())}")
    for h in handles: h.remove()
    del model; gc.collect(); torch.cuda.empty_cache()
    return cap
...
def main():
    ...
    hf_cap = run_hf(mp, hf_inputs, enc_out, args.device, args.dtype, args.layer)
    torch_cap = run_torch(mp, hf_inputs, enc_out, args.device, args.dtype, args.layer)

    print(f"\n{'='*70}")
    print(f"Layer {args.layer} attention sub-component bisection:")
    print(f"{'='*70}")
    for key in ["after_layernorm", "q_proj_out", "k_proj_out", "v_proj_out",
                 "q_norm_out", "k_norm_out", "o_proj_out"]:
        if key in hf_cap and key in torch_cap:
            cmp(key, hf_cap[key], torch_cap[key])
        else:
            print(f"  {key}: MISSING (hf={key in hf_cap}, torch={key in torch_cap})")
