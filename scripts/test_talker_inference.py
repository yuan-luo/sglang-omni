...
class MockRadixAttention(nn.Module):
    ...
    def forward(self, q, k, v, forward_batch, save_kv_cache=False):
        ...
        if self.num_kv_groups > 1:
            k = k.unsqueeze(2).expand(-1, -1, self.num_kv_groups, -1).reshape(total_tokens, self.num_heads, self.head_dim)
            v = v.unsqueeze(2).expand(-1, -1, self.num_kv_groups, -1).reshape(total_tokens, self.num_heads, self.head_dim)
...
class _Nil:
    ...
    @property
    def shape(self): return torch.Size([0])
...
def test_inference():
    ...
    config = Qwen3OmniMoeTalkerConfig(
        ...
        text_config={
            ...
            "num_experts": 8, "moe_intermediate_size": 256,
            "shared_expert_intermediate_size": 256, "vocab_size": 1000,
    ...
    thinker_embeds = torch.randn(batch_size, seq_len, config.thinker_hidden_size,
                                  device=device, dtype=dtype)
    inputs_embeds = model.prepare_input_embeds(thinker_embeds=thinker_embeds)
    print(f"    {inputs_embeds.shape}  (expect [{batch_size}, {seq_len}, {config.text_config.hidden_size}])")
    ...
    hidden_states = model.forward(
        ...
        forward_batch=fb, inputs_embeds=inputs_embeds_flat,
    )
    print(f"    hidden_states: {hidden_states.shape}  (expect [{total_tokens}, {config.text_config.hidden_size}])")
    assert hidden_states.shape == (total_tokens, config.text_config.hidden_size)

    # ---- Step 3: compute_logits (layer-0) ----
    print("\n[3] compute_logits")
    logits = model.compute_logits(hidden_states)
    print(f"    logits: {logits.shape}  (expect [{total_tokens}, {config.text_config.vocab_size}])")
    ...
    print(f"    result_codes:  {result_codes.shape}  (expect [{batch_size}, {config.num_code_groups}, {seq_len}])")
    print(f"    summed_embeds: {summed_embeds.shape}  (expect [{batch_size}, {seq_len}, {config.code_predictor_config.hidden_size}])")
    ...
    assert (result_codes >= 0).all() and (result_codes < config.code_predictor_config.vocab_size).all(), \
        f"Codes out of range: min={result_codes.min()}, max={result_codes.max()}"
