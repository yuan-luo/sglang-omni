...
"""Check: is SDPA deterministic with identical input? Run torch thinker twice."""
...
def run_torch(model_path, hf_inputs, enc_out, device, dtype, run_id):
    ...
    print(f"[Torch run {run_id}] Loading ...")
    model = Qwen3OmniTorchThinker(model_path, device=device, dtype=dtype)
    cap = {}
    layer = model.thinker.layers[0]
    h = []
    h.append(layer.register_forward_pre_hook(lambda m,a,kw=None: cap.__setitem__("input", a[0].detach().cpu())))
    h.append(layer.self_attn.register_forward_hook(lambda m,a,o: cap.__setitem__("attn", (o[0] if isinstance(o,tuple) else o).detach().cpu())))
    h.append(layer.register_forward_hook(lambda m,a,o: cap.__setitem__("output", (o[0] if isinstance(o,tuple) else o).detach().cpu())))
    ids = hf_inputs["input_ids"]
    mask = hf_inputs.get("attention_mask", torch.ones_like(ids))
    kw = _kw(hf_inputs, enc_out)
    print(f"[Torch run {run_id}] Forward ...")
...
def main():
    ...
    print(f"\n{'='*60}")
    print("Torch vs Torch (same model, same input, two runs):")
    print(f"{'='*60}")
    for k in ["input","attn","output"]:
