...
def create_sglang_thinker_executor(
    ...
    *,
    ...
    def _enqueue_chunk(request_id: str, hidden, metadata=None):
        fn = enqueue_fn_holder["fn"]
        if fn is not None:
            fn(request_id, hidden, metadata=metadata)
    ...
    def _request_builder(payload: StagePayload):
        ...
        data.capture_prompt_hidden = bool(speech_enabled)
        return data
    ...
    def _set_chunk_enqueue_fn(fn) -> None:
        enqueue_fn_holder["fn"] = fn

    setattr(executor, "set_chunk_enqueue_fn", _set_chunk_enqueue_fn)
    return executor
...
def make_thinker_stream_adapter(chunk_enqueue_fn=None):
    """Create a Thinker stream adapter with optional hidden state side-channel.

    Args:
        chunk_enqueue_fn: Optional callable(request_id, hidden_tensor, metadata=None).
            Called synchronously on each decode step that produces hidden states.
            Typically ChunkTransferAdapter.enqueue.
    ...
    def _stream_adapter(request, output):
        if chunk_enqueue_fn is not None and output.extra is not None:
            ...
            if prefill_hidden is not None:
                embed, layer_hidden = _split_dual_layer_hidden(prefill_hidden)
                if embed is not None:
                    chunk_enqueue_fn(
                        request.request_id,
                        embed,
                        metadata={
                            "kind": "prompt_prefill_hidden",
        ...
        # Side-channel: send hidden states to chunk transfer (sync)
        if chunk_enqueue_fn is not None and output.extra is not None:
...
def build_talker_ar_request(payload: StagePayload) -> dict:
    ...
    if chunks:
        thinker_hidden_states = torch.stack([c.tensor for c in chunks], dim=0)
        thinker_token_ids = [
            c.metadata["token_id"]
            for c in chunks
