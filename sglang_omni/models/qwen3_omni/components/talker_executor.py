...
from sglang_omni.models.qwen3_omni.pipeline.talker_input import (
...
class TalkerStreamingExecutor(Executor):
    ...
    def __init__(
        ...
        *,
        ...
        self._engine_feedback_mailbox = ChunkMailbox(max_pending=4096)
    ...
    async def add_request(self, payload: StagePayload) -> None:
        ...
        if self._chunk_mailbox is None:
            raise RuntimeError("Talker executor requires a chunk mailbox")
        ...
        thinker_chunks, pending_feedbacks, thinker_done = (
            await self._collect_initial_chunks(request_id, min_thinker_chunks=1)
        )
        if request_id in self._aborted:
            return
    ...
    async def _collect_initial_chunks(
        ...
        *,
        ...
        thinker_chunks: list[ChunkItem] | None = None,
        pending_feedbacks: list[torch.Tensor] | None = None,
        thinker_done: bool = False,
    ) -> tuple[list[ChunkItem], list[torch.Tensor], bool]:
        ...
        while len(thinker_chunks) < min_thinker_chunks and not thinker_done:
            item = await self._chunk_mailbox.get_with_source(request_id)
        ...
        *,
        ...
        if thinker_chunks:
            try:
                ...
                logger.exception(
                    "Prompt prefill build failed for %s; falling back to assistant-only prefill",
                    request_id,
                )

        if len(thinker_chunks) >= 4:
            try:
                return self._build_assistant_prefill_request(
                    payload=payload,
                    request_id=request_id,
                    thinker_chunks=thinker_chunks,
                    thinker_done=thinker_done,
                ...
                logger.exception(
                    "Assistant prefill build failed for %s; falling back to linear prefill",
                    request_id,
                )

        thinker_hidden_states = (
            torch.stack([c.tensor for c in thinker_chunks], dim=0)
            if thinker_chunks
            else torch.empty(0)
        )
        thinker_token_ids = [
    ...
    def _build_assistant_prefill_request(
        self,
        *,
        payload: StagePayload,
        request_id: str,
        ...
        tts_bos_embed, tts_eos_embed, tts_pad_embed = self._get_tts_special_embeds()
        assistant_embed = torch.stack([c.tensor for c in thinker_chunks[:4]], dim=0).to(
        ...
        self._log_assistant_component_summary(
            ...
            tts_bos_embed=tts_bos_embed,
            tts_pad_embed=tts_pad_embed,
            assembled_input_embeds=assistant_result["input_embeds"],
        )
        self._log_prefill_summary(
            request_id=request_id,
            prefill_type="assistant",
            input_ids=assistant_result["input_ids"],
            input_embeds=assistant_result["input_embeds"],
            source_token_ids=assistant_token_ids,
            assistant_token_ids=assistant_token_ids,
        )
        ...
        data = build_sglang_talker_request(
            ...
            codec_eos_id=sampling_cfg["codec_eos_id"],
            suppress_tokens=sampling_cfg["suppress_tokens"],
            talker_input_embeds=assistant_result["input_embeds"],
            talker_input_ids=assistant_result["input_ids"],
            input_embeds_are_projected=True,
            trailing_text_hidden=trailing_list,
            tts_pad_embed=tts_pad_embed[0].detach().cpu(),
            thinker_chunks_done=thinker_done,
    ...
    def _reconstruct_prompt_states(
        ...
        captured_prompt = self._load_captured_prompt_hidden(state, prompt_ids.shape[0])
        if captured_prompt is not None:
            prompt_embed, prompt_hidden = captured_prompt
        else:
        ...
        if isinstance(model_inputs, dict) and captured_prompt is None:
    ...
    def _load_captured_prompt_hidden(
        self,
        state: PipelineState,
        expected_seq_len: int,
    ) -> tuple[torch.Tensor, torch.Tensor] | None:
        thinker_out = state.thinker_out or {}
        extra_outputs = thinker_out.get("extra_model_outputs", {})
        if not isinstance(extra_outputs, dict):
            return None

        prompt_hidden = extra_outputs.get("prompt_prefill_hidden_states")
        if not isinstance(prompt_hidden, dict):
            return None

        embed = prompt_hidden.get("embed")
        if not isinstance(embed, torch.Tensor):
            embed = prompt_hidden.get(0)
        ...
        if (
            ...
            logger.warning(
                ...
                tuple(embed.shape),
                tuple(layer_hidden.shape),
                expected_seq_len,
            )
            return None

        return (
            embed.to(device=self._device, dtype=self._dtype),
            layer_hidden.to(device=self._device, dtype=self._dtype),
        )
    ...
    def _load_prompt_token_embeddings(self, token_ids: torch.Tensor) -> torch.Tensor:
        ...
        unique_rows = _load_thinker_embedding_rows(
            self._resolved_model_path, unique_ids.tolist()
        ).to(device=self._device, dtype=self._dtype)
    ...
    def _apply_prompt_modality(
        ...
        *,
        ...
        prompt_hidden[mask] = feature_tensor
    ...
    def _extract_thinker_chunk_token_ids(
        self, thinker_chunks: list[ChunkItem]
    ) -> torch.Tensor:
        token_ids: list[int] = []
    ...
    def _chunk_layer_hidden_or_embed(self, chunk: ChunkItem) -> torch.Tensor:
        metadata = chunk.metadata if isinstance(chunk.metadata, dict) else {}
            ...
            while request_id not in self._aborted:
                ...
                try:
                    item = await asyncio.wait_for(
                        self._chunk_mailbox.get_with_source(request_id),
                        timeout=0.01,
                    )
    ...
    def _route_inbound_item(
        self,
        request_id: str,
        item: ChunkItem | ChunkSignal,
        *,
        thinker_chunks: list[ChunkItem] | None,
        ...
        if isinstance(item, ChunkSignal):
            if item.error is not None:
                raise item.error
            if item.is_chunks_done and self._is_thinker_source(item):
                if update_request:
                    self._mark_thinker_done(request_id)
                return True
            return False

        if self._is_prompt_prefill_hidden(item):
            self._append_prompt_prefill_hidden(request_id, item)
            return False
        ...
        pending_feedbacks.append(item.tensor.detach().cpu())
        return False

    def _is_thinker_source(self, item: ChunkItem | ChunkSignal) -> bool:
    ...
    @staticmethod
    def _is_prompt_prefill_hidden(item: ChunkItem | ChunkSignal) -> bool:
        metadata = getattr(item, "metadata", None)
        return (
            isinstance(metadata, dict)
            and metadata.get("kind") == "prompt_prefill_hidden"
        )

    def _append_prompt_prefill_hidden(self, request_id: str, chunk: ChunkItem) -> None:
        payload = self._payloads.get(request_id)
        if payload is None:
            return

        metadata = chunk.metadata if isinstance(chunk.metadata, dict) else {}
        prompt_embed = chunk.tensor.detach().cpu()
        layer_hidden = metadata.get("layer_hidden")
        if isinstance(layer_hidden, torch.Tensor):
        ...
        extra_outputs = thinker_out.setdefault("extra_model_outputs", {})
        store = extra_outputs.setdefault("prompt_prefill_hidden_states", {})

        existing_embed = store.get("embed")
        store["embed"] = (
            prompt_embed
            if not isinstance(existing_embed, torch.Tensor)
            else torch.cat([existing_embed, prompt_embed], dim=0)
        )

        if layer_hidden is not None:
            layer_key = self._accept_hidden_layer
            existing_layer = store.get(layer_key)
            store[layer_key] = (
                layer_hidden
                if not isinstance(existing_layer, torch.Tensor)
                else torch.cat([existing_layer, layer_hidden], dim=0)
            )

        state.thinker_out = thinker_out
        payload.data = state.to_dict()

    def _append_trailing_chunk(self, request_id: str, chunk: ChunkItem) -> None:
        ...
        if not isinstance(trailing, list):
            return
        trailing.append(self._project_assistant_chunk(chunk).cpu())
    ...
    def _mark_thinker_done(self, request_id: str) -> None:
        ...
        if isinstance(trailing, list) and isinstance(tts_eos_embed, torch.Tensor):
            trailing.append(tts_eos_embed.detach().cpu())
